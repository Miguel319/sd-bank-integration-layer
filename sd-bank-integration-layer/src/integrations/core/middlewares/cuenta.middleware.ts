...
// @access Private
const getCuentaById = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const estado = Estado.getInstance();
...
// @access Public
const createCuenta = asyncHandler(
  async (
    req: Request,
    res: Response,
    next: NextFunction
...
// @access Private
const updateCuenta = asyncHandler(
  async (
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (estado.getArriba()) {
      ...
      try {
        ...
        if (!fieldsToUpdt)
          return next(
            new ErrorResponse(
              "Debe proveer al menos un campo a actualizar.",
              400
            )
          );
...
// @route  PUT /api/v1/cuentas/:_id
// @access Private
const deleteCuenta = asyncHandler(
  async (
    req: Request,
    res: Response,
    next: NextFunction
