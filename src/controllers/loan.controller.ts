...
export const createLoan = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    res
      .status(201)
      .json({ success: true, message: "Loan created succesfully!." });
  }
);
...
export const payLoanByUser = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!loanBelongsToUser) {
      return next(
        new ErrorResponse(
          `Provided loan ${_id} doesn't belong to this user.`,
          400
        )
      );
    }
    ...
    if ((loan as any).paid === (loan as any).total) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Loan was completely paid." });
    }
    ...
    if (montoNumber > (loan as any).remaining) {
      return next(
        new ErrorResponse("You can't exceed the remaining amount", 400)
      );
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Loan updated succesfully!." });
  }
);
